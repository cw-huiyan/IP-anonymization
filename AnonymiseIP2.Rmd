...
The impact of IP anonymization is evaluated by comparing the number of sessions attributed to each location from both properties. Here we assume that geo-location identified with full IP address is 100% accurate and compare the anonymized IP version aganist it to assess the impact of IP anonymization. While in fact, it is worth noting that the method of using IP to infer geo-location is already defected thus not accurate due to the nature of IP address. [Study](https://radical-analytics.com/case-study-accuracy-precision-of-google-analytics-geolocation-4264510612c0) shows the precision varies from a few meters to 250km (Hamel, 2016).
...
Besides country level discrepancies, continent, region, and city level geo-location attribution were also calculated. The same procedure was done for USB Flashdrive's US site. You may find the complete code in [here](https://github.com/cw-huiyan/IP-anonymization/blob/master/AnonymiseIP2.Rmd).
...
```
In addition, UK site continent level overall variations is `r overall_diff_uk_continent`.
...
In addition, US site continent level overall variations is `r overall_diff_us_continent`.
...
*  **City(17%~21%) > Region/State(2%~4%) > Country(1%) > Continent(0.7%)**
